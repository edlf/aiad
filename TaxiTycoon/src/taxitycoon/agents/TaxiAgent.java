...
public class TaxiAgent extends SimAgent {
	...
	public static void createAgentMap(ArrayList<Pair<Integer, Integer>> roads,
		...
		for (int i = 0; i < _mapSize.getValue0(); i++) {
			for (int j = 0; j < _mapSize.getValue1(); j++) {
				...
				case 3:
					_intersectionPositions
							.add(new Pair<Integer, Integer>(i, j));
					break;
	...
	public boolean relativeMove(Pair<Integer, Integer> delta) {
		Pair<Integer, Integer> newPosition = new Pair<Integer, Integer>(
				_currentPosition.getValue0() + delta.getValue0(),
				_currentPosition.getValue1() + delta.getValue1());
		...
		if (_map[newPosition.getValue0()][newPosition.getValue1()] == _mapRoad) {
			gasReduce();
			return _move(new Pair<Integer, Integer>(newPosition.getValue0(),
					newPosition.getValue1()));
		}
	...
	public LinkedList<Pair<Integer, Integer>> getShortestPathTo(
		...
		/*
		 * Create cost map and fill with 0 possible locations, -1 impossible
		 * locations
		 */
		int costMap[][] = new int[_mapSize.getValue0()][_mapSize.getValue1()];
		for (int i = 0; i < _mapSize.getValue0(); i++) {
			for (int j = 0; j < _mapSize.getValue1(); j++) {
				if (_map[i][j] == _mapRoad || _map[i][j] == _mapTaxiStop
						|| _map[i][j] == _mapRefuel) {
					costMap[i][j] = 0;
				} else {
					costMap[i][j] = -1;
		...
		int startI = _currentPosition.getValue0(), startJ = _currentPosition
				.getValue1();
		...
		while (!queue.isEmpty()) {
			...
			int currentCost = costMap[currentPos.getValue0()][currentPos
					.getValue1()];

			ArrayList<Pair<Integer, Integer>> nextPositions = new ArrayList<>();
			nextPositions.add(new Pair<Integer, Integer>(
					currentPos.getValue0() + 1, currentPos.getValue1()));
			nextPositions.add(new Pair<Integer, Integer>(
					currentPos.getValue0(), currentPos.getValue1() + 1));
			nextPositions.add(new Pair<Integer, Integer>(
					currentPos.getValue0() - 1, currentPos.getValue1()));
			nextPositions.add(new Pair<Integer, Integer>(
					currentPos.getValue0(), currentPos.getValue1() - 1));
		...
		while (!currentPos.equals(_currentPosition)) {
			...
			int currentLevel = costMap[currentPos.getValue0()][currentPos
					.getValue1()];
