...
public class PassengerAgent extends SimAgent {
	...
	public PassengerAgent(Pair<Integer, Integer> initialPos, Pair<Integer, Integer> destination) {
		if (_isPointWithinBonds(initialPos)) {
			this._startPosition = initialPos;
		} else {
			System.out
	...
	public Pair<Integer, Integer> getNearestStop() {
		...
		for (Pair<Integer, Integer> stop : _stopsPositions) {
			if (nearestStop == null) {
				nearestStop = stop;
				currentBest = SimAgent.getCostBetweenTwoPoints(
						_currentPosition, stop);
			} else {
				int cost = SimAgent.getCostBetweenTwoPoints(_currentPosition,
						stop);
				if (cost < currentBest) {
					nearestStop = stop;
					currentBest = cost;
	...
	public void printStats() {
		System.out
				.println(getLocalName()
						+ " walking: "
						+ TaxiTycoonLauncher
								.roundDouble(getTicksWalkingPercent())
						+ "% waiting: "
						+ TaxiTycoonLauncher
								.roundDouble(getTicksWaitingPercent())
						+ "% in taxi: "
						+ TaxiTycoonLauncher
								.roundDouble(getTicksInTaxiPercent()) + "%");
	}
